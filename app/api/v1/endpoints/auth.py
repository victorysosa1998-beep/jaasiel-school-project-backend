...
def login(body: LoginRequest, request: Request, db: Session = Depends(get_db)):
    ...
    if role == "student":
        # NOTE: is_active is checked AFTER the password, not inside the same
        # query — otherwise a deactivated student gets the exact same
        # generic "Invalid credentials" as someone typing the wrong
        # password, with no way to tell the two apart. Staff logins already
        # distinguish "wrong credentials" (401) from "account inactive"
        # (403); students should behave the same way. Password is still
        # checked first so an unauthenticated guesser can't use this to
        # probe which usernames are active.
        student = db.query(Student).filter(Student.username == identifier).first()
        if not student or not verify_password(body.password, student.password_hash):
            raise HTTPException(401, "Invalid student credentials")
        if not student.is_active:
            raise HTTPException(403, "This student account has been deactivated. Please contact the school office.")
        sub = f"s:{student.id}"
        access  = create_access_token({"sub": sub, "role": "student"})
        refresh = create_refresh_token({"sub": sub})
        db.add(LoginSession(student_id=student.id, user_name=student.full_name,
